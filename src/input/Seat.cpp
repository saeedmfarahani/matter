...
void Seat::inputDeviceUnplugged(LInputDevice *device) { L_UNUSED(device) }
...
void Seat::configureInputDevices() noexcept {
  if (compositor()->inputBackendId() != LInputBackendLibinput) return;

  libinput_device *dev;

  for (LInputDevice *device : inputDevices()) {
    if (!device->nativeHandle()) continue;
