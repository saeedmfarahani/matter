...
void Pointer::pointerMoveEvent(const LPointerMoveEvent &event) {
  ...
  if (seat()->dnd()->dragging() &&
    ...
    else if (seat()->dnd()->action() == LDND::Move ||
             seat()->dnd()->action() == LDND::Ask)
  ...
  /* Normally, we should restore the cursor if no surface is focused. However,
   * during toplevel move or resize sessions, clients typically update the
   * cursor to indicate the edge or corner being resized, but the cursor
   * position might move outside the surface, causing it to lose focus.*/
