...
Systemd::Systemd() noexcept {
  ...
  if (ret < 0) {
    LLog::error("Systemd: Failed to connect to the user bus: %s.",
                strerror(-ret));
    goto failOpen;
  }
...
UInt32 Systemd::getServiceMainPID(const std::string &serviceName) noexcept {
  ...
  ret = sd_bus_call_method(bus, "org.freedesktop.systemd1",
                           ...
                           &error, &reply, "s", serviceName.c_str());
  ...
  UInt32 pid;
  ret = sd_bus_get_property_trivial(bus, "org.freedesktop.systemd1", unitPath,
...
bool Systemd::restartXdgDesktopPortal() noexcept {
  ...
  if (system("dbus-update-activation-environment --systemd WAYLAND_DISPLAY "
             "XDG_CURRENT_DESKTOP=wlroots") != 0)
    return false;

  sd_bus_message *reply{nullptr};
  Int32 ret = sd_bus_call_method(
      bus, "org.freedesktop.systemd1", "/org/freedesktop/systemd1",
      "org.freedesktop.systemd1.Manager", "RestartUnit", NULL, &reply, "ss",
      "xdg-desktop-portal.service", "replace");
  if (ret < 0) {
    LLog::error("Systemd: Failed to restart xdg-desktop-portal.service: %s.",
                strerror(-ret));
    return false;
  }
...
Int32 Systemd::serviceEventHandler(sd_bus_message *m, void *data,
  ...
  if (strcmp(unitName, "xdg-desktop-portal-wlr.service") != 0) return 0;

  Systemd *systemd{static_cast<Systemd *>(data)};
  sd_bus_error error{SD_BUS_ERROR_NULL};

  ret = sd_bus_get_property_trivial(
      systemd->bus, "org.freedesktop.systemd1", unitPath,
      "org.freedesktop.systemd1.Service", "MainPID", &error, 'u',
      &systemd->xdgDesktopPortalWlrPID);
  if (ret < 0) {
    LLog::error("Systemd: Failed to get xdg-desktop-portal-wlr MainPID: %s.",
                error.message);
    sd_bus_error_free(&error);
    return 0;
  }
