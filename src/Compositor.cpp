...
void Compositor::uninitialized() {
  /* All factory objects, client resources and backends are still available at
   * this point. Textures should be released here before the graphic backend is
   * uninitialized */
...
LFactoryObject *Compositor::createObjectRequest(LFactoryObject::Type objectType,
                                                const void *params) {
  if (objectType == LFactoryObject::Type::LOutput) return new Output(params);

  if (objectType == LFactoryObject::Type::LClient) return new Client(params);

  if (objectType == LFactoryObject::Type::LSurface) return new Surface(params);
  ...
  if (objectType == LFactoryObject::Type::LSeat) return new Seat(params);

  if (objectType == LFactoryObject::Type::LPointer) return new Pointer(params);

  if (objectType == LFactoryObject::Type::LKeyboard)
    return new Keyboard(params);

  if (objectType == LFactoryObject::Type::LTouch) return new Touch(params);

  if (objectType == LFactoryObject::Type::LClipboard)
    return new Clipboard(params);

  if (objectType == LFactoryObject::Type::LDND) return new DND(params);

  if (objectType == LFactoryObject::Type::LSessionLockManager)
    return new SessionLockManager(params);

  /* Returning nullptr tells Louvre to use the default class for the given
   * LFactoryObject type */
  return nullptr;
}
...
bool Compositor::globalsFilter(LClient *client, LGlobal *global) {
  ...
  if (global->isType<Protocols::ScreenCopy::GScreenCopyManager>() && systemd &&
      systemd->xdgDesktopPortalWlrPID != 0) {
    pid_t clientPID;
    client->credentials(&clientPID);

    if ((UInt32)clientPID != systemd->xdgDesktopPortalWlrPID) return false;
  }
#endif
