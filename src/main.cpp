...
int main(int, char *[]) {
   ...
   * when forking after LCompositor is started, as libseat and other subsystems
   * might leak resources and interfere with the session if not handled
   * properly. */
  LLauncher::startDaemon();

  Compositor compositor;

  /* You can load a custom input and graphics backend here. For more details,
   * see the LCompositor documentation. By default, if the WAYLAND_DISPLAY
   * environment variable is set, the Wayland backends will be loaded,
   * otherwise, if launched from a free TTY, the DRM and Libinput backends will
   * be used.*/
