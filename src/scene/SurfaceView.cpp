...
bool SurfaceView::nativeMapped() const noexcept {
  /* Keep the default implementation (it checks if surface() is nullptr, if the
   * surface is mapped and so on) */
  return LSurfaceView::nativeMapped() &&

         /* If the session is locked, we only display surfaces from the
            locking-client */
