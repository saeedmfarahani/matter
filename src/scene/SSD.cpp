...
SSD::SSD(ToplevelRole *toplevel) noexcept : LLayerView(), toplevel(toplevel) {
...
SSD::~SSD() {
  toplevel->setExtraGeometry({.left = 0, .top = 0, .right = 0, .bottom = 0});
...
void SSD::updateGeometry() noexcept {
  ...
  titlebar.setSize(size().width() + extraGeo.left + extraGeo.right,
                   extraGeo.top);

  close.setPos(extraGeo.left, (extraGeo.top - close.size().h()) / 2);
  maximize.setPos(close.nativePos().x() + close.size().w() + extraGeo.left,
                  close.nativePos().y());
  minimize.setPos(
      maximize.nativePos().x() + maximize.size().w() + extraGeo.left,
      maximize.nativePos().y());
...
const LPoint &SSD::nativePos() const noexcept { return toplevel->rolePos(); }
