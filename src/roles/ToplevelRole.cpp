...
ToplevelRole::ToplevelRole(const void *params) noexcept
  ...
  resizeSession().setOnBeforeUpdateCallback(
...
void ToplevelRole::setMinimizedRequest() { surface()->setMinimized(true); }
...
void ToplevelRole::atomsChanged(LBitset<AtomChanges> changes,
                                const Atoms &prev) {
  ...
  if (changes.check(WindowGeometryChanged) && ssd) updateSSD = true;

  if (!changes.check(StateChanged)) goto skip;

  stateChanges = state() ^ prev.state;

  if (stateChanges.check(Activated)) updateSSD = true;
    ...
    if (maximized()) {
      if (exclusiveOutput()) {
        ...
        surface()->setPos(exclusiveOutput()->pos() +
                          exclusiveOutput()->availableGeometry().pos());
  ...
  if (stateChanges.check(Fullscreen | Maximized) &&
      !pendingConfiguration().state.check(Fullscreen | Maximized))
    setExclusiveOutput(nullptr);

skip:
  if (ssd && updateSSD) ssd->updateGeometry();
}

void ToplevelRole::titleChanged() { /* See title() */ }

void ToplevelRole::appIdChanged() { /* See appId() */ }
